...
1. modify the Booking class so that it also stores the mobile number and number of guests. The names of these variables should be:
   - `mobile`, and
   - `no_of_guests`
2. adjust the code in the `while True:` loop so that also requests the mobile number and number of guests associated with the booking
3. adjust the other lines of code so that these additional details are stored and printed when the `print(book)` instruction executes
3. add internal documentation which meets with the standards, described [here](https://sites.google.com/bssc.edu.au/bssd34/unit-3/area-of-study-1-programming/internal-documentation).
